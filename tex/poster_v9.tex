...
\placeholder{CBraMod 原论文任务复现}{%
    主表：BCIC2A / FACED / ISRUC-S1 / Physionet-MI / SEED-V / SHU-MI / MDD / TUEV\\
    每项报告 3-seed avg bAcc、best bAcc、论文参考值、结论。\\
    \small (等待队友复现结果填入)}
...
\textbf{LP (冻结 Backbone)：}NeuroGate 在睡眠任务上提升明显——HMC (+0.0773)、ISRUC-S1 (+0.0972)，说明人工 EEG 先验能够补充 Frozen CBraMod Embedding 中不易被线性头读出的信息。

\textbf{FT (端到端微调)：}在 selected-12 数据集上 LP Gated Fusion 相比 Backbone-only 平均提升 +0.0389 BA (40/48 为正)；FT 下提升较小但总体为正 (+0.0078 BA，9/12 为正)。

\textbf{可讲结论：}NeuroGate 的主证据在 LP，说明显式神经先验能补充 Frozen Foundation Embedding；FT 证据可作为辅助支持。

\placeholder{NeuroGate LP/FT 实验结果}{%
    LP：HMC / ISRUC-S1 / MDD / Physionet-MI 对比柱状图。\\
    FT：selected-12 数据集上 LP/FT Gated Fusion vs Backbone-only。\\
    \small (等待队友复现结果)}
