...
\newcommand{\modelsizecompare}{%
    ...
    \colorbox{mygray!8}{\parbox{0.95\columnwidth}{%
        \vspace{0.6em}
        {\Large\bfseries\color{myblue}模型规模的“实际大小”对比（建议用参数量/计算量）}\par
        \vspace{0.6em}
        \begin{tikzpicture}[x=1cm,y=1cm]
            % 说明：下面矩形面积按“参数量(M)”粗略比例绘制；请用你们复现代码统计的真实数值替换注释中的参数量。
            \fill[mygreen!30] (0,0) rectangle (1.2,0.6);
            \node[anchor=west] at (1.4,0.3) {\large EEGNet \quad \textcolor{mygray}{(示例：0.02M params)}};

            \fill[myorange!30] (0,-1.0) rectangle (3.0,-0.2);
            \node[anchor=west] at (3.2,-0.6) {\large EEG Conformer \quad \textcolor{mygray}{(示例：1.5M params)}};

            \fill[mypurple!25] (0,-2.2) rectangle (6.0,-1.0);
            \node[anchor=west] at (6.2,-1.6) {\large CBraMod \quad \textcolor{mygray}{(示例：20M params)}};
        \end{tikzpicture}
        \vspace{0.4em}\par
        {\small\color{mygray}注：把示例数值替换为你们统计的真实参数量/计算量后，这张图就能表达“实际大小对比”。}\par
        \vspace{0.6em}
    }}
    \end{center}
}
...
\placeholderh{模型架构对比（放前面）}{%
    EEGNet / EEG Conformer / CBraMod 结构示意图。\\
    展示三个模型的关键模块和架构差异；建议保留统一的输入窗口与通道数标注。}{4.5cm}
...
\placeholderh{模型性能对比（占比更大）}{%
    三个模型在多个数据集上的性能比较。\\
    指标：Accuracy / F1 Score / Cohen's Kappa。\\
    \small (等待队友复现结果)}{6.0cm}

\subsection{训练效率比较}

\placeholderh{训练效率对比（占比更大）}{%
    三个模型的参数量 / 训练时间 / 推理速度对比。\\
    \small (等待队友复现结果)}{4.8cm}
...
\placeholderh{NeuroGate 结构示意图（未来工作只保留一张图）}{%
    CBraMod Backbone + 神经科学先验\\
    $\downarrow$ Gated Fusion\\
    增强表征 (泛化 + 可解释性)\\
    \small (等待绘制)}{3.5cm}

{\small\color{mygray}注：NeuroGate 的初步实验结果与对比图请放到 `v8.2` 的 Results 区块，避免挤占 Future Work。}
